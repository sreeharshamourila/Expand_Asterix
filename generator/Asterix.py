...
from data.renderer.Asterix import AsterixRenderer
from data.features.Asterix import AsterixFeatures

class AsterixGenerator(Generator):
  """
  ...
  def __init__(self, config):
    super(AsterixGenerator, self).__init__(config, 
      renderer=True,
      features=True
    )

  def _init_features(self):
    self.features = AsterixFeatures(self.config)

  def _init_renderer(self):
    self.renderer = AsterixRenderer(self.config)
  ...
  def _sample_attributes(self, attr, node_class):
    ...
    """
    out = super(AsterixGenerator, self)._sample_attributes(attr, node_class)
    # First run to get standard attributes

    # Handle special case of node textures
    if node_class in ["demon","hero","target"]:
